...
struct sample {
    ...
    uint64_t pool_used_bytes_post_comp;
...
static int run_one(const struct options *opts, const char *dataset, int run_id, int warmup, struct sample *s)
{
    ...
    s->pool_used_bytes_post_comp = after_comp.pool_bytes_live;
...
static int write_csv_header(FILE *fp)
{
    return fprintf(fp,
                   ...
                   "compressed_bytes_post_comp,slot_bytes_post_comp,"
                   "pool_used_bytes_post_comp,pool_free_bytes_post_comp,pool_frag_bytes_post_comp,"
                   ...
                   "ratio_e2e_post_comp,ratio_admitted_post_comp,ratio_codec_post_comp,"
...
static int append_csv_row(FILE *fp, const struct options *opts, const char *dataset, const struct sample *s)
{
    ...
    uint64_t unadmitted_bytes = 0;
    uint64_t e2e_den = 0;
    double ratio_e2e = 0.0;
    double ratio_admitted = safe_ratio_u64(
        s->arena_stats_post_comp.logical_input_bytes,
        s->pool_used_bytes_post_comp
    );
    double ratio_codec = safe_ratio_u64(
        s->arena_stats_post_comp.logical_input_bytes,
        s->compressed_bytes_post_compress
    );

    if (s->arena_stats_post_comp.total_input_bytes_attempted >= s->arena_stats_post_comp.logical_input_bytes) {
        unadmitted_bytes =
            s->arena_stats_post_comp.total_input_bytes_attempted - s->arena_stats_post_comp.logical_input_bytes;
    }
    e2e_den = unadmitted_bytes + s->pool_used_bytes_post_comp;
    ratio_e2e = safe_ratio_u64(s->arena_stats_post_comp.total_input_bytes_attempted, e2e_den);
    ...
    if (fprintf(fp,
                "%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%.6f,%.6f,%.6f,%.6f,"
                "%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ","
                "%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ","
                "%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 ",%" PRIu64 "\n",
...
static void print_header(void)
{
    ...
    printf("ratio_e2e ratio_admitted ratio_codec admit_rate incompressible\n");
    printf("------------ --- ---- ------------------------- ------------ ------------------ ------------------- ");
    printf("--------- ------------- ----------- ---------- ------------\n");
}
...
static void print_row(const char *dataset, const struct sample *s, const struct options *opts)
{
    ...
    printf("%-12s %3d %4d %-25s %12" PRIu64 " %18" PRIu64 " %19" PRIu64 " %9.3f %13.3f %11.3f %10.3f %12" PRIu64 "\n",
           dataset,
           s->run_id,
           s->warmup,
           phase_model_name(opts->phase_model),
