...
static int fail_errno(const char *what, const char *path)
{
    if (path != NULL) {
        fprintf(stderr,
                "process_pager_damon: %s failed path=%s errno=%d (%s)\n",
                what,
                path,
                errno,
                strerror(errno));
        ...
        fprintf(stderr,
                "process_pager_damon: %s failed errno=%d (%s)\n",
                what,
                errno,
                strerror(errno));
    }
    ...
    const struct pager_damon_config *cfg)
{
    ...
    if (path_join(path, sizeof(path), scheme_path, "access_pattern/sz/min") != 0 || write_u64_file(path, 1) != 0) {
    ...
    if (path_join(path, sizeof(path), scheme_path, "access_pattern/nr_accesses/min") != 0 || write_u64_file(path, 0) != 0) {
        return fail_errno("write access_pattern nr_accesses/min", path);
    }
    if (path_join(path, sizeof(path), scheme_path, "access_pattern/nr_accesses/max") != 0 || write_u64_file(path, UINT32_MAX) != 0) {
        return fail_errno("write access_pattern nr_accesses/max", path);
    }
    if (path_join(path, sizeof(path), scheme_path, "access_pattern/age/min") != 0 || write_u64_file(path, 0) != 0) {
        return fail_errno("write access_pattern age/min", path);
    }
    if (path_join(path, sizeof(path), scheme_path, "access_pattern/age/max") != 0 || write_u64_file(path, UINT32_MAX) != 0) {
...
{
    char path[PATH_MAX];
    uint64_t nr_kdamonds;

    if (damon == NULL || cfg == NULL || pid <= 0 || region_start >= region_end) {
        errno = EINVAL;
        return -1;
    }
...
static int load_one_obs(const char *dir_path, struct pager_damon_region_obs *out_obs)
{
    char path[PATH_MAX];
...
static int list_tried_regions(struct pager_damon_snapshot *out_snapshot)
{
    ...
    while ((de = readdir(dir)) != NULL) {
        ...
        struct pager_damon_region_obs obs;
        ...
        if (out_snapshot->count == cap) {
            size_t new_cap;
            struct pager_damon_region_obs *tmp;
    ...
    struct pager_damon_snapshot *out_snapshot)
{
    ...
    min_period_ns = (uint64_t)(cfg->read_tick_ms ? cfg->read_tick_ms : 200) * 1000000ULL;
    ...
    if (write_text_file(DAMON_ADMIN_ROOT "/kdamonds/0/state", "update_schemes_tried_regions") != 0) {
        return fail_errno("write state=update_schemes_tried_regions", DAMON_ADMIN_ROOT "/kdamonds/0/state");
    }
    if (list_tried_regions(out_snapshot) != 0) {
        return fail_errno("read tried_regions", DAMON_ADMIN_ROOT "/kdamonds/0/contexts/0/schemes/0/tried_regions");
    }
    if (write_text_file(DAMON_ADMIN_ROOT "/kdamonds/0/state", "clear_schemes_tried_regions") != 0) {
        pager_damon_snapshot_free(out_snapshot);
        return fail_errno("write state=clear_schemes_tried_regions", DAMON_ADMIN_ROOT "/kdamonds/0/state");
    }
