...
int main(int argc, char **argv)
{
    ...
    if (use_mem_arena) {
        ...
        if (mem_arena_region_alloc(arena, len, "anon_streamer", &region_id, &raw_buf) != 0) {
    ...
    if (common.json) {
        ...
        printf("anon_streamer region_mb=%d idle_ms=%d duration_sec=%d passes=%" PRIu64 " elapsed_ms=%.3f use_mem_arena=%d use_process_pager=%d compress_policy=%s external_compress_triggers=%" PRIu64,
               region_mb,
               idle_ms,
               common.duration_sec,
               passes,
    ...
    if (use_mem_arena) {
        if (arena_stats_json != NULL) {
            if (write_arena_stats_json(arena_stats_json, "anon_streamer", &arena_stats) != 0) {
                mem_arena_destroy(arena);
                return 1;
            }
        }
